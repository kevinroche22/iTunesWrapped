...
# Load raw iTunes data, ensuring dateAdded is read in as date type
...
iTunesInfo['songNameAndArtist'] = iTunesInfo['song'] + \
    ', ' + iTunesInfo['albumArtist']

# Add "albumNameAndArtist" variable
iTunesInfo['albumNameAndArtist'] = iTunesInfo['album'] + \
    ', ' + iTunesInfo['albumArtist']
...
conditions = [
    ...
    iTunesInfo['genre'].str.contains("((?i)R\&B|soul|rap|hip.{0,1}hop)") == False]
...
# Simplify genres
iTunesInfo['genre'] = np.select(conditions,
                                values,
                                default="task")

# Filter on music added in 2021
iTunesInfo = iTunesInfo[(iTunesInfo['dateAdded'] >= pd.to_datetime('2021-01-01')) &
                        (iTunesInfo['dateAdded'] <= pd.to_datetime('2021-12-31'))]
...
style = [
    dict(selector="th", props=thProps)]
...
mostListenedSongs = iTunesInfo.sort_values(by='playCount',
                                           kind='mergesort',
...
listensPerDay = iTunesInfo[(iTunesInfo['daysSinceDownload'] > 30)].sort_values(by='playFrequency',
                                                                               kind='mergesort',
                                                                               ascending=False).head(10)

# Determine 10 most listened artists that released in 2021
mostListenedArtists = pd.DataFrame(iTunesInfo.groupby(
    ['albumArtist'])['playCount'].sum().sort_values(ascending=False).head(10))

# Determine 10 most listened albums released in 2021
mostListenedAlbums = pd.DataFrame(iTunesInfo.groupby(['albumNameAndArtist'])[
    'playCount'].sum().sort_values(ascending=False).head(10))

# Determine 10 most skipped songs released in 2021
mostSkips = iTunesInfo.sort_values(by='skips',
                                   kind='mergesort',
                                   ascending=False).head(10)

# Determine how many hours I spent listening to my top 10 songs
hoursPerSong = pd.DataFrame(iTunesInfo.groupby(['song', 'albumArtist'])[
                            'totalSeconds'].sum().sort_values(ascending=False).head(10))
hoursPerSong['totalSeconds'] = hoursPerSong['totalSeconds'].div(3600)
hoursPerSong = hoursPerSong.rename({'totalSeconds': 'totalHours'},
                                   axis='columns')
hoursPerSong.style.set_table_styles(style)

# Determine how many hours I spent listening to my top 10 artists
hoursPerArtist = pd.DataFrame(iTunesInfo.groupby(['albumArtist'])[
                              'totalSeconds'].sum().sort_values(ascending=False).head(10))
hoursPerArtist['totalSeconds'] = hoursPerArtist['totalSeconds'].div(3600)
hoursPerArtist = hoursPerArtist.rename({'totalSeconds': 'totalHours'},
                                       axis='columns')
hoursPerArtist.style.set_table_styles(style)

# Determine how many hours I spent listening to my top 10 albums
hoursPerAlbum = pd.DataFrame(iTunesInfo.groupby(['album', 'albumArtist'])[
                             'totalSeconds'].sum().sort_values(ascending=False).head(10))
hoursPerAlbum['totalSeconds'] = hoursPerAlbum['totalSeconds'].div(3600)
hoursPerAlbum = hoursPerAlbum.rename({'totalSeconds': 'totalHours'},
                                     axis='columns')
hoursPerAlbum.style.set_table_styles(style)

# Determine time spent listening across genres
hoursPerGenre = pd.DataFrame(iTunesInfo.groupby(
    ['genre'])['totalSeconds'].sum().sort_values(ascending=False).head(3))
hoursPerGenre['totalSeconds'] = hoursPerGenre['totalSeconds'].div(3600)
hoursPerGenre = hoursPerGenre.rename({'genre': 'genre', 'totalSeconds': 'totalHours'},
                                     axis='columns')

# Total minutes&hours listened listened
...
plt.pie(hoursPerGenre['totalHours'],
        labels=genreLabels,
        autopct='%1.1f%%',
        shadow=True,
...
plt.tight_layout()
plt.savefig("/Users/kevinroche22/PythonData/iTunesWrapped/plots/genreBreakdown.png")
plt.show()

# Plot most listened songs
mostListenedSongs = mostListenedSongs.sort_values('playCount')
plt.barh(mostListenedSongs['songNameAndArtist'],
         mostListenedSongs['playCount'],
         color = 'tomato')
plt.title("Most Listened Songs of 2021")
plt.xlabel('Play Count')
plt.box(False)
plt.savefig("/Users/kevinroche22/PythonData/iTunesWrapped/plots/mostListenedSongs.png", bbox_inches="tight")
plt.show()

# Plot most listened artists
mostListenedArtists = mostListenedArtists.sort_values('playCount')
plt.barh(mostListenedArtists.index, 
         mostListenedArtists['playCount'],
         color = 'tomato')
plt.title("Most Listened Artists Of 2021")
plt.xlabel('Combined Play Count (Songs)')
plt.box(False)
plt.savefig("/Users/kevinroche22/PythonData/iTunesWrapped/plots/mostListenedArtists.png", bbox_inches="tight")
plt.show()

# Plot most listened albums
mostListenedAlbums = mostListenedAlbums.sort_values('playCount')
plt.barh(mostListenedAlbums.index, 
         mostListenedAlbums['playCount'],
         color = 'tomato')
plt.title("Most Listened Albums of 2021")
plt.xlabel('Combined Play Count (Songs)')
plt.box(False)
plt.savefig("/Users/kevinroche22/PythonData/iTunesWrapped/plots/mostListenedAlbums.png", bbox_inches="tight")
plt.show()
